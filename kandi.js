(function ($) {
  ...
  function updateGround() {
    ...
    for (var i = 0; i < ground.length; i++) {
      ...
      var angle;
      if (
        player.minDist(ground[i]) <= player.height / 2 + platformWidth / 2 &&
        (angle =
          (Math.atan2(player.y - ground[i].y, player.x - ground[i].x) * 180) /
          Math.PI) > -130 &&
        angle < -50
      ) {
        player.isJumping = false;
  ...
  function spawnEnemySprites() {
    if (
      ...
      platformLength > 5 &&
      (enemies.length
        ? canvas.width - enemies[enemies.length - 1].x >= platformWidth * 3 ||
          canvas.width - enemies[enemies.length - 1].x < platformWidth
        : true)
    ) {
      enemies.push(
        new Sprite(
  ...
  function animate() {
    if (!stop) {
      ...
      // draw the score
      ctx.fillText("Resistance Tokens: " + score, canvas.width - 300, 30);
      ...
      if (
        ticker > Math.floor(platformWidth / player.speed) * player.speed * 20 &&
        player.dy !== 0
      ) {
        player.speed = bound(++player.speed, 0, 15);
        player.walkAnim.frameSpeed =
          Math.floor(platformWidth / player.speed) - 1;
